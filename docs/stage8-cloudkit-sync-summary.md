...
The rollout is development-only. As stabilized in Stage 12B, Debug builds start synchronization automatically outside tests, while tests and both Release platforms default transport off through one shared policy. No Production CloudKit schema was deployed as part of this stage or Stage 12B.
...
- Record types: `TDNote` V1/V2, `TDTask` V1, and advisory `TDClient` V1
- Mac and iPhone entitlements select the Development environment and the same container.
- The iPhone target enables the `remote-notification` background mode and registers for remote notifications when the shared policy activates non-test Debug transport.
...
The historical Stage 8 content contract has since gained `TDNote` V2 color fields and `TDClient` V1. The exact current fields, CloudKit types, optionality, record-name rules, zone, and compatibility behavior are source-generated in [the Development contract manifest](development-cloudkit-contract-manifest.md). Record names remain content-free stable IDs.
...
Before testing, select a development team for both targets, verify both entitlements resolve to `iCloud.studio.cuatro.tildone`, and leave the container environment at Development. Current Debug apps synchronize automatically outside tests. On the Mac only, add `TILDONE_ALLOW_LOCAL_WORKSPACE_ADOPTION=1` when explicitly testing adoption of pre-sync local data; remove it for all non-adoption scenarios.
...
- No schema-marker record was added. `TDClient` is the single advisory, content-free third record type; domain `schemaVersion` fields provide per-record compatibility checks.
