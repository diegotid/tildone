...
Current post-Stage-12B safeguards are:

- CloudKit sync is automatic only for non-test Debug processes; tests and both
  Release platform builds are transport-off through the shared default policy.
...
- `TildoneSync` maps the durable outbox to `CKSyncEngine` pending changes and
  synchronizes `TDNote` V1/V2 and `TDTask` V1 records in private custom zone
  `TildoneUserData`; advisory device status uses `TDClient` V1.
...
| Offline edits, force-quit, relaunch, and reconnect | Passed in both directions | During the iPhone offline gate, local mutations survived force-quit, offline relaunch, and an additional mutation. The independent Home Screen launch initially showed `icloud.slash` because it did not inherit Stage 10's historical scheme opt-in setting, which Stage 12B has since removed. No data/reset action was taken. Relaunching the existing app from Xcode with that historical setting cleared the disabled state, drained the durable outbox, and converged on Mac. The Mac-direction gate likewise retained multiple mutations through an opt-in Debug force-quit/relaunch plus an additional post-relaunch mutation, then drained its outbox and converged exactly on iPhone after reconnect. The owner confirmed stable final relaunches without loss, duplication, or resurrection in both directions. |
...
2. Use current Debug builds; synchronization starts automatically outside tests.
...
6. Return to CloudKit Console and confirm that the private database contains
   record types `TDNote`, `TDTask`, and `TDClient`, and custom zone `TildoneUserData`.
