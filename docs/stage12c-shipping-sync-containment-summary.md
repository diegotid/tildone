...
- When iCloud needs the user’s attention, the same note-level position instead shows an orange `exclamationmark.icloud` button. Its plain-language label says that iCloud is not syncing right now and the notes remain safe on the Mac; pressing it opens the existing status window. Attention takes precedence over the local-only symbol, and the indicator updates on already-open notes. The iPhone collection UI is unchanged.
...
- macOS hosted `TildoneTests` on the final current tree: **32 tests executed, 0 failures, 2 opt-in tests skipped**. This includes retained-source combine/outbox coverage, per-account relaunch choice persistence, active/paused/attention presentation, same-window confirmation-gated actions, accessible actionable Mac-only and attention note indicators, retention of the Tildone menu-bar icon with a rendered attention badge, removal of reset hatches, deterministic scheme assertions, Mac CRUD, and legacy migration coverage.
...
4. Exercise forced local presentation of zone-reset and incompatible-data attention states. Verify copy, accessibility, that the cloud-with-exclamation button appears on every open Mac note and opens the existing status window, and that no reset/reseed/overwrite action exists.
