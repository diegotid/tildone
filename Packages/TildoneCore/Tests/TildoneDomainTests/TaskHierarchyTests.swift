...
final class TaskHierarchyTests: XCTestCase {
    func testRecursiveLeavesProgressSubtreesAndParentsShareOnePreorderModel() throws {
        ...
        XCTAssertEqual(TaskHierarchy.insertionIndexAfterSubtree(startingAt: 0, in: tasks), 4)
        XCTAssertEqual(TaskHierarchy.insertionIndexAfterSubtree(startingAt: 1, in: tasks), 3)
        XCTAssertEqual(TaskHierarchy.insertionIndexAfterSubtree(startingAt: 2, in: tasks), 3)
